...
export function buildReflectionMemoryPrompt(args: {
  ...
  return [
    ...
    "Inputs:",
    "- CURRENT_MEMORY: the current compact profile; it may be stale, too literal, or missing nuance.",
    "- RECENT_SENT_EMAILS: what the system recently showed the reader; use this to detect mismatch, repetition, and neglected range.",
    "- RECENT_REPLY_EMAILS: the reader's recent language; this is strong evidence, though one message can still be temporary.",
    "Interpret conservatively:",
    "- Prefer no_change unless recent sent/reply evidence clearly shows the memory is stale, cluttered, mis-sectioned, or missing a reinforced durable pattern.",
    "- Do not mistake temporary curiosity, one narrow named item, or one emotional spike for durable identity.",
    "- Use sent emails for mismatch/repetition and replies for durable preference shifts.",
    ...
    "- If rewriting, make a targeted section/bullet-level cleanup pass; do not paraphrase everything unnecessarily.",
    "- PERSONALITY should hold durable intellectual/style traits, cognitive archetypes, learning preferences, and filtering preferences.",
    "- ACTIVE_INTERESTS should hold subject areas, domains, and recurring areas of coverage the reader wants more of.",
    "- If a line is mainly about how the reader thinks, filters, learns, or prefers material, it belongs in PERSONALITY.",
    "- If a line is mainly about what subject or domain the reader wants covered, it belongs in ACTIVE_INTERESTS.",
    "- RECENT_FEEDBACK should stay short-horizon, concise, and non-duplicative.",
    "- Keep narrow named items reversible unless clearly reinforced.",
    "- Use rewrites to clean memory, not just to add content: merge overlapping lines, remove obvious redundancy, and compress repeated signals into cleaner durable statements.",
    "Decision rules:",
    "- Choose no_change when the current memory is still coherent enough and a rewrite would be speculative or cosmetic.",
    "- Choose rewrite when the current memory is stale, cluttered, mis-sectioned, overly literal, overly broad, or missing reinforced patterns visible in the recent emails.",
    "- If you are unsure whether a pattern is durable, keep memory stable.",
    "Discovery brief rules:",
    "- The discovery brief is secondary; leave arrays empty when nothing meaningful stands out.",
    "- reinforceTopics: topics to lean into today.",
    "- avoidPatterns: patterns to avoid today.",
