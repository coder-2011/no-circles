...
- `canonical memory text`: required sectioned format for `interest_memory_text` (`PERSONALITY`, `ACTIVE_INTERESTS`, `RECENT_FEEDBACK`).
- `active-interest lanes`: two priority levels encoded inside `ACTIVE_INTERESTS` bullets; plain bullets are core interests and `[side]` bullets are lighter-weight or reversible interests.
- `serendipity lane`: reserved discovery budget for adjacent topics that are not direct active-interest topics but still fit the reader's profile.
