...
- Environment variable: `RESEND_API_KEY` (used to fetch inbound email text by `email_id` from the Resend received-email API when webhook payload omits inline text).
...
7. Ignore obvious automated `noreply`/`do-not-reply` sender addresses before any content fetch or DB work.
8. Resolve reply text:
   - use inline `data.text` when present
   - if missing and `data.email_id` exists, fetch message text from Resend via `emails.receiving.get`
   - for fetched email content, extract newest reply segment and trim quoted-thread/history markers before memory merge
9. Ignore empty reply text after resolution.
10. Look up user by sender email.
11. If sender is unknown, send best-effort guidance auto-reply asking the user to reply from their subscribed email (using thread headers when available to suggest address), then return `ignored`.
12. Generate updated memory via reply processor.
13. In a DB transaction:
...
14. Return `updated` or `ignored`.
15. `GET` returns `405 METHOD_NOT_ALLOWED` (route is not browser-navigable for processing).
