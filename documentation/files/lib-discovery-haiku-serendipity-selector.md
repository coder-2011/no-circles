...
8. Uses elevated temperature (`0.85`) to encourage non-redundant adjacent topic picks.
9. Parses strict JSON output shape:
- `{"topics":["..."]}`
10. Applies minimal safety filtering on model outputs (dedupe, drop active overlaps, normalize).
11. Returns no serendipity topics when model/env/API is unavailable; caller continues with core lane only.
