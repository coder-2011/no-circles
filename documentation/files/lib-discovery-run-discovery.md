...
4. Run Haiku selector per topic (when topic has >1 candidate) and reorder candidates so selected link is ranked first.
   - selector failures are warning-only.
5. Normalize results into discovery candidates.
...
6. Apply optional candidate include hook to normalized candidates.
7. Dedupe globally via canonical URLs.
8. Exclude soft-suppressed topics from the primary quality pool.
9. Apply quality filters before winner selection:
...
10. Enforce per-domain cap (`maxPerDomain`) on quality-filtered pool.
11. Perform strict one-winner-per-topic selection using weighted topic-local score:
   - `0.65 * exaNorm + 0.35 * highlightNorm` (weights renormalized when one signal is missing).
12. Backfill to target count (default `10`) in staged order:
...
13. Build `diversityCard` on final output with hard thresholds for topic/domain spread.
14. Enforce target-count contract:
