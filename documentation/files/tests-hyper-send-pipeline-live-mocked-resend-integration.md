...
4. Assert final mocked email payload includes `Quote of the Day` in both HTML and text.
5. Verify persisted post-send DB state:
...
6. Write run artifacts under `logs/hyper/full-system/<run-id>/send-pipeline-result.txt`.
7. Clean up temporary DB rows.
